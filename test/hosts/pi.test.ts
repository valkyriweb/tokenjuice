...
import { doctorPiExtension, installPiExtension, uninstallPiExtension } from "../../src/index.js";
...
describe("installPiExtension", () => {
  ...
  it("uninstalls a generated extension and removes the legacy sibling runtime", async () => {
    const home = await createTempDir();
    const extensionDir = join(home, "extensions");
    const extensionPath = join(extensionDir, "tokenjuice.js");
    const legacyRuntimePath = join(extensionDir, "tokenjuice-runtime.js");
    ...
    await writeFile(
      extensionPath,
      "/* generated by tokenjuice v0.0.0 */\nexport default createTokenjuicePiExtension({});\n",
      "utf8",
    );
    await writeFile(legacyRuntimePath, "legacy\n", "utf8");

    const result = await uninstallPiExtension(extensionPath);

    expect(result).toEqual({ extensionPath, removed: true, restoredBackup: false });
    expect(await readOptional(extensionPath)).toBeUndefined();
    expect(await readOptional(legacyRuntimePath)).toBeUndefined();
  });

  it("restores the pi extension backup when uninstalling a generated extension", async () => {
    const home = await createTempDir();
    const extensionPath = join(home, "tokenjuice.js");
    const backupPath = `${extensionPath}.bak`;
    ...
    await writeFile(backupPath, "export default function customExtension() {}\n", "utf8");

    const result = await uninstallPiExtension(extensionPath);

    expect(result).toEqual({ extensionPath, backupPath, removed: true, restoredBackup: true });
    expect(await readFile(extensionPath, "utf8")).toBe("export default function customExtension() {}\n");
  });

  it("reports a restored custom pi extension as disabled", async () => {
    const home = await createTempDir();
    const extensionPath = join(home, "tokenjuice.js");
    const customSource = "export default function customExtension() {}\n";
    ...
    await writeFile(`${extensionPath}.bak`, customSource, "utf8");

    await uninstallPiExtension(extensionPath);
    const report = await doctorPiExtension(extensionPath);

    expect(report.status).toBe("disabled");
    expect(report.issues).toEqual([]);
  });

  it("preserves and restores the original custom pi backup across reinstall", async () => {
    const home = await createTempDir();
    const extensionPath = join(home, "tokenjuice.js");
    const customSource = "export default function customExtension() {}\n";

    await writeFile(extensionPath, customSource, "utf8");

    const firstInstall = await installPiExtension(extensionPath);
    const secondInstall = await installPiExtension(extensionPath);
    const uninstall = await uninstallPiExtension(extensionPath);

    expect(firstInstall.backupPath).toBe(`${extensionPath}.bak`);
    expect(secondInstall.backupPath).toBe(`${extensionPath}.bak.1`);
    ...
    expect(await readFile(`${extensionPath}.bak`, "utf8")).toBe(customSource);
    expect(await readFile(`${extensionPath}.bak.1`, "utf8")).toContain("createTokenjuicePiExtension");
    expect(await readFile(extensionPath, "utf8")).toBe(customSource);
  });

  it("restores the newest custom pi backup when older backups remain", async () => {
    const home = await createTempDir();
    const extensionPath = join(home, "tokenjuice.js");
    const oldCustomSource = "export default function oldCustomExtension() {}\n";
    const newCustomSource = "export default function newCustomExtension() {}\n";

    await writeFile(extensionPath, oldCustomSource, "utf8");
    await installPiExtension(extensionPath);
    await uninstallPiExtension(extensionPath);
    await writeFile(extensionPath, newCustomSource, "utf8");
    await installPiExtension(extensionPath);
    ...
    expect(uninstall).toEqual({
      extensionPath,
      backupPath: `${extensionPath}.bak.1`,
      removed: true,
      restoredBackup: true,
    });
    expect(await readFile(extensionPath, "utf8")).toBe(newCustomSource);
  });

  it("does not restore a tokenjuice-generated pi extension backup", async () => {
    const home = await createTempDir();
    const extensionPath = join(home, "tokenjuice.js");
    const backupPath = `${extensionPath}.bak`;
    const generatedSource = "/* generated by tokenjuice v0.0.0 */\nexport default createTokenjuicePiExtension({});\n";

    await writeFile(extensionPath, generatedSource, "utf8");
    await writeFile(backupPath, generatedSource, "utf8");

    const result = await uninstallPiExtension(extensionPath);

    expect(result).toEqual({ extensionPath, removed: true, restoredBackup: false });
    expect(await readOptional(extensionPath)).toBeUndefined();
    expect(await readFile(backupPath, "utf8")).toBe(generatedSource);
  });

  it("does not remove a custom pi extension", async () => {
    const home = await createTempDir();
    const extensionPath = join(home, "tokenjuice.js");
    const source = "export default function customExtension() {}\n";

    await writeFile(extensionPath, source, "utf8");

    const result = await uninstallPiExtension(extensionPath);

    expect(result).toEqual({ extensionPath, removed: false, restoredBackup: false });
    expect(await readFile(extensionPath, "utf8")).toBe(source);
  });
